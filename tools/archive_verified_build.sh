...
# Sync documentation/metadata commits that may have landed while CI was building.
...
# Confirm the frozen source did not change while the build was running.
echo "84fb1b957290abdf2464a97b35b81f2d1e8e976ce9c067ecaf571c8c19f22c68  $SOURCE_ZIP" | sha256sum -c -

# Browsable exact source tree. This is safe to refresh because it is derived from the frozen ZIP.
rm -rf source/v0.1
mkdir -p source/v0.1
unzip -q "$SOURCE_ZIP" -d source/v0.1

# v0.1 is frozen: archive its repository binary only once. Future CI runs verify
# a fresh build but do not replace the already archived/debug-signed baseline.
if [[ ! -f releases/v0.1/K_G1_Discovery_v0_1.apk ]]; then
