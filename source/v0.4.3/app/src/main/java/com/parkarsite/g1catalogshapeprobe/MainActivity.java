package com.parkarsite.g1catalogshapeprobe;
...
public final class MainActivity extends Activity {
    ...
    private enum Phase { IDLE, ENTER_SENT, DISCOVERING, CONNECTING, CONNECTED, CATALOG_GET, EXIT_SENT, CLEANUP, COMPLETE }
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Catalog Shape Probe"); t.setTextSize(24); root.addView(t);
        TextView n=new TextView(this); n.setText("G4B2: repeats the same single catalog GET, but only characterizes the returned media.config body safely: encoding/BOM, JSON top-level type, known protocol keys and counts. No filename/path values and no media-file request."); n.setPadding(0,dp(8),0,dp(16)); root.addView(n);
        statusView=new TextView(this); statusView.setTextSize(16); root.addView(statusView);
        runButton=new Button(this); runButton.setText("Run G4B2 response-shape probe"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then run G4B2 once.");
    }
    ...
    private void runProbe(){
        ...
        expectedP2pName=null; credentialPassword=null; failurePending=false; ipEventObserved=false; failureReason=null; glassesClientIp=null;
        p2pGroupFormed=false; phoneIsGroupOwner=false; catalogGetAttempted=false; catalogSuccess=false; httpRequestCount=0; observed73Events.clear(); phase=Phase.IDLE;
        runButton.setEnabled(false);copyButton.setEnabled(false);shareButton.setEnabled(false);
        append("K G1 CATALOG RESPONSE-SHAPE PROBE REPORT");
        append("Generated: "+isoNow());
        append("App version: 0.4.3");
        append("Gate: G4B2 one read-only local media catalog GET + safe response classification");
        append("BLE writes allowed: P2P enter once + transfer exit once");
        append("HTTP allowed: EXACTLY ONE GET to /files/media.config after passive 0x73/0x08 IP resolution");
        ...
        append("Filename/path logging: DISABLED");
        append("Media-file download/mutation: NOT IMPLEMENTED");
        append("");
    ...
    private void maybeStartCatalogGet(){
        ...
        cancelTimeout(); catalogGetAttempted=true; httpRequestCount=1; phase=Phase.CATALOG_GET;
        append(""); append("READ-ONLY LOCAL CATALOG");
        append("Method: GET");
        ...
        setStatus("Reading media.config once…");
    ...
    private void fetchCatalogOnce(String ip){
        ...
        try{
            ...
            handler.post(()->{
                ...
                append("Catalog SHA-256: "+summary.sha256);
                ...
                append("Line count: "+summary.lineCount);
                append("First non-whitespace token: "+summary.firstToken);
                append("Last non-whitespace token: "+summary.lastToken);
                append("Raw JSON parse: "+summary.rawJsonType);
                append("Normalized JSON parse: "+summary.normalizedJsonType);
                append("Root key count: "+summary.rootKeyCount);
                append("file_list present: "+summary.fileListPresent);
                append("file_list type: "+summary.fileListType);
                append("file_list entries: "+summary.totalItems);
                append("Known item keys present: "+summary.knownItemKeys);
                append("Unknown item key count: "+summary.unknownItemKeyCount);
                append("Entries with non-empty f path: "+summary.validPaths);
                append("Extension summary: "+summary.extensionSummary);
                append("Filename/path values logged: NO");
                append("Media-file GET requests: 0");
                catalogSuccess=summary.catalogUsable;
                setStatus(summary.catalogUsable?"Catalog structure identified. Exiting transfer mode…":"Response characterized. Exiting transfer mode…");
                sendExit();
            });
        }catch(Exception e){
    ...
    private static CatalogSummary classifyCatalog(byte[] body,String contentType,String contentEncoding){
        CatalogSummary out=new CatalogSummary();
        out.bodyBytes=body==null?0:body.length;
        out.sha256=sha256(body==null?new byte[0]:body);
        ...
        out.lineCount=countLines(raw);
        out.firstToken=tokenClass(raw,true);
        out.lastToken=tokenClass(raw,false);

        Object rawValue=parseJsonValue(raw);
        out.rawJsonType=jsonType(rawValue);

        String normalized=stripBom(raw).trim();
        Object normalizedValue=parseJsonValue(normalized);
        out.normalizedJsonType=jsonType(normalizedValue);

        Object root=normalizedValue!=null?normalizedValue:rawValue;
        inspectRoot(root,out);
        return out;
    }

    private static void inspectRoot(Object root,CatalogSummary out){
        if(root instanceof JSONObject){
            JSONObject obj=(JSONObject)root;
            out.rootKeyCount=obj.length();
            out.fileListPresent=obj.has("file_list");
            if(out.fileListPresent){
                Object v=obj.opt("file_list");
                out.fileListType=jsonType(v);
                if(v instanceof JSONArray){
                    inspectFileList((JSONArray)v,out);
                }else if(v instanceof String){
                    Object nested=parseJsonValue(((String)v).trim());
                    if(nested instanceof JSONArray){
                        out.fileListType="STRING->ARRAY";
                        inspectFileList((JSONArray)nested,out);
                    }else if(nested instanceof JSONObject){
                        out.fileListType="STRING->OBJECT";
                    }
                }
            }
        }else if(root instanceof JSONArray){
            out.rootKeyCount=0;
            out.fileListPresent=false;
            out.fileListType="ROOT_ARRAY";
            inspectFileList((JSONArray)root,out);
        }else if(root instanceof String){
            Object nested=parseJsonValue(((String)root).trim());
            if(nested instanceof JSONObject || nested instanceof JSONArray){
                inspectRoot(nested,out);
                out.normalizedJsonType="STRING->"+jsonType(nested);
            }
        }
    }

    private static void inspectFileList(JSONArray list,CatalogSummary out){
        out.totalItems=list.length();
        if(list.length()>CATALOG_MAX_ITEMS){
            out.fileListType=out.fileListType+" (OVER_ITEM_CAP)";
            return;
        }
        int valid=0,unknown=0;
        TreeMap<String,Integer> extensions=new TreeMap<>();
        TreeSet<String> knownSeen=new TreeSet<>();
        Set<String> known=new HashSet<>(Arrays.asList("c","e","f","h","s","t","w"));
        for(int i=0;i<list.length();i++){
            ...
            while(it.hasNext()){
                String k=it.next();
                if(known.contains(k))knownSeen.add(k); else unknown++;
            }
            String path=item.optString("f","");
            if(path!=null&&!path.isEmpty()){
                valid++;
                String ext=extensionOnly(path);
                extensions.put(ext,extensions.getOrDefault(ext,0)+1);
            }
        }
        out.validPaths=valid;
        out.knownItemKeys=knownSeen.isEmpty()?"none":String.join(",",knownSeen);
        out.unknownItemKeyCount=unknown;
        out.extensionSummary=summarizeExtensions(extensions);
        out.catalogUsable=true;
    }

    private static Object parseJsonValue(String s){
        if(s==null||s.isEmpty())return null;
        try{
            JSONTokener t=new JSONTokener(s);
            Object v=t.nextValue();
            char trailing=t.nextClean();
    ...
    private static String jsonType(Object v){
        ...
        if(v instanceof JSONObject)return "OBJECT";
    ...
    private static String tokenClass(String s,boolean first){
        if(s==null||s.isEmpty())return "NONE";
        int i=first?0:s.length()-1;
        int step=first?1:-1;
        while(i>=0&&i<s.length()){
            char ch=s.charAt(i);
            if(ch=='\uFEFF'||Character.isWhitespace(ch)){i+=step;continue;}
            if(ch=='{')return "OBJECT_START";
            if(ch=='}')return "OBJECT_END";
            if(ch=='[')return "ARRAY_START";
            if(ch==']')return "ARRAY_END";
            if(ch=='\"')return "STRING_QUOTE";
            if(ch=='<' )return "ANGLE_BRACKET";
            if(ch==0)return "NUL";
            if(ch<128)return "OTHER_ASCII";
            return "NON_ASCII";
        }
        return "NONE";
    }
    ...
    private static String sha256(byte[] data){
        try{
            ...
            StringBuilder b=new StringBuilder();
            for(byte x:h)b.append(String.format(Locale.US,"%02x",x&255));
            return b.toString();
        }catch(Exception e){
            return "<unavailable>";
        }
    ...
    private static final class CatalogSummary{
        int bodyBytes,lineCount,rootKeyCount,totalItems,validPaths,unknownItemKeyCount;
        String sha256="<unavailable>",contentType="<none>",contentEncoding="<none>",bom="NONE";
        String firstToken="NONE",lastToken="NONE",rawJsonType="FAIL",normalizedJsonType="FAIL";
        String fileListType="ABSENT",knownItemKeys="none",extensionSummary="none";
        boolean utf8Valid,fileListPresent,catalogUsable;
    }
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            String reason=failureReason==null?"G4B did not complete.":failureReason;
            cleanupRuntime(false);
            append("");append("SUMMARY");append("Exact target peer matched: "+targetPeerMatched);appendIpSummary();append("HTTP GET requests: "+httpRequestCount);append("Media-file GET requests: 0");append("Credentials logged/persisted: NO");append("Filename/path values logged: NO");append("G4B2 RESULT: FAILED — "+reason);append("END REPORT");
    ...
    private void finishSuccess(){
        cleanupRuntime(false); append("");append("SUMMARY");append("Exact target peer matched: "+targetPeerMatched);append("P2P group formed: "+p2pGroupFormed);append("Phone is group owner: "+phoneIsGroupOwner);appendIpSummary();append("HTTP GET requests: "+httpRequestCount);append("Catalog GET attempted: "+catalogGetAttempted);append("Catalog parsed successfully: "+catalogSuccess);append("Media-file GET requests: 0");append("Credentials logged/persisted: NO");append("Filename/path values logged: NO");
        append(catalogSuccess?"G4B2 RESULT: CATALOG STRUCTURE IDENTIFIED — REVIEW PHYSICAL REPORT":"G4B2 RESULT: RESPONSE CHARACTERIZED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        reportFinished=true;phase=Phase.COMPLETE;setStatus("G4B2 response-shape probe complete. Review the report.");enableActions();
    }
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G4B2 Catalog Shape Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G4B2 Catalog Shape Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G4B2 report"));}
