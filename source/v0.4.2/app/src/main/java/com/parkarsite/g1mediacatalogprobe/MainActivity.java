package com.parkarsite.g1mediacatalogprobe;
...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Media Catalog Probe"); t.setTextSize(24); root.addView(t);
        TextView n=new TextView(this); n.setText("G4B: repeats the proven P2P association flow, resolves the glasses IPv4 from passive 0x73/0x08, then performs exactly one bounded local GET of /files/media.config. No media-file download or mutation."); n.setPadding(0,dp(8),0,dp(16)); root.addView(n);
        statusView=new TextView(this); statusView.setTextSize(16); root.addView(statusView);
        runButton=new Button(this); runButton.setText("Run G4B catalog probe"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then run G4B once.");
    }
    ...
    private void runProbe(){
        ...
        append("K G1 MEDIA CATALOG PROBE REPORT");
        append("Generated: "+isoNow());
        append("App version: 0.4.2");
        append("Gate: G4B one read-only local media catalog GET");
    ...
    private void fetchCatalogOnce(String ip){
        ...
        try{
            ...
            CatalogSummary summary=parseCatalog(new String(body,StandardCharsets.UTF_8));
            handler.post(()->{
                if(reportFinished)return;
                append("HTTP status: 200");
                append("Catalog bytes received: "+body.length);
                append("JSON root: object");
                append("file_list present: YES");
                append("file_list entries: "+summary.totalItems);
                append("Entries with non-empty f path: "+summary.validPaths);
                append("Observed item keys: "+summary.keySummary);
                append("Extension summary: "+summary.extensionSummary);
                append("Filename/path values logged: NO");
                append("Media-file GET requests: 0");
                catalogSuccess=true;
                setStatus("Catalog listing validated. Exiting transfer mode…");
                sendExit();
            });
        }catch(Exception e){
    ...
    private static CatalogSummary parseCatalog(String json) throws JSONException{
        JSONObject root=new JSONObject(json);
        JSONArray list=root.optJSONArray("file_list");
        if(list==null)throw new JSONException("file_list missing");
        if(list.length()>CATALOG_MAX_ITEMS)throw new JSONException("too many catalog items");
        int valid=0;
        ...
        for(int i=0;i<list.length();i++){
            ...
            while(it.hasNext()){
                String k=it.next();
                if(k!=null&&k.matches("[A-Za-z0-9_]{1,32}"))keys.add(k);
            }
        ...
        return new CatalogSummary(list.length(),valid,summarizeKeys(keys),summarizeExtensions(extensions));
    }
    ...
    private static String summarizeKeys(SortedSet<String> keys){
        if(keys.isEmpty())return "none";
        StringBuilder b=new StringBuilder(); int count=0;
        for(String k:keys){if(count++>=16){b.append(", …");break;} if(b.length()>0)b.append(", ");b.append(k);}
        return b.toString();
    }
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            ...
            cleanupRuntime(false);
            append("");append("SUMMARY");append("Exact target peer matched: "+targetPeerMatched);appendIpSummary();append("HTTP GET requests: "+httpRequestCount);append("Media-file GET requests: 0");append("Credentials logged/persisted: NO");append("Filename/path values logged: NO");append("G4B RESULT: FAILED — "+reason);append("END REPORT");
    ...
    private void finishSuccess(){
        ...
        append(catalogSuccess?"G4B RESULT: READ-ONLY CATALOG LISTING VALIDATED — REVIEW PHYSICAL REPORT":"G4B RESULT: NO VALID CATALOG — STOP AND REVIEW");
        append("END REPORT");
        reportFinished=true;phase=Phase.COMPLETE;setStatus("G4B catalog probe complete. Review the report.");enableActions();
    }
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G4B Media Catalog Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G4B Media Catalog Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G4B report"));}
