...
JAVA="$SRC/java/com/parkarsite/g1cataloglineprobe/MainActivity.java"
MANIFEST="$SRC/AndroidManifest.xml"
fail=0
bad(){ if grep -RniE "$1" "$SRC" >/tmp/g4b3_hits 2>/dev/null; then echo "FAIL: $2"; cat /tmp/g4b3_hits; fail=1; else echo "PASS: $2 absent"; fi; }
...
bad 'JSONObject|JSONArray|JSONTokener|PtPFileModel|file_list' 'wrong JSON parser branch'
...
bad 'MessageDigest|Catalog SHA-256|sha256\(' 'catalog fingerprint/hash logging'
bad 'append\([^\n]*\+[[:space:]]*(raw|trimmed|line)([^A-Za-z0-9_]|$)' 'catalog content value logging'
...
grep -Fq 'new BufferedReader(new StringReader(raw))' "$JAVA" || { echo "FAIL: line-list parser missing"; fail=1; }
grep -Fq 'while((line=reader.readLine())!=null)' "$JAVA" || { echo "FAIL: readLine loop missing"; fail=1; }
...
grep -Fq 'Catalog raw body logged: NO' "$JAVA" || { echo "FAIL: raw-body redaction declaration missing"; fail=1; }
