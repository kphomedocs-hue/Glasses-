...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Disposable Photo Probe v0.5.1"); t.setTextSize(24); root.addView(t);
        ...
        runButton=new Button(this); runButton.setText("Start G5.1 baseline"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then start the G5.1 baseline.");
    }
    ...
    private void startTransportCycle(boolean fresh){
        ...
        if(fresh){
            ...
            append("Gate: G5.1 disposable JPG download with exact Cyan inventory + catalog-readiness parity");
            ...
            if(!baselineReady||baselineCatalog.isEmpty()){
                fail("Baseline is unavailable. Restart G5.1 from Phase A.");
                return;
            }
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            String reason=failureReason==null?"G5.1 did not complete.":failureReason;
            cleanupRuntime(false);
            append("");
            append("SUMMARY");
            ...
            append("G5.1 RESULT: FAILED — "+reason);
            append("END REPORT");
            baselineCatalog.clear();
            baselineReady=false;
            reportFinished=true;
    ...
    private void finishBaselinePhase(){
        ...
        phase=Phase.COMPLETE;
        setStatus("Phase A complete. Now use the glasses to capture EXACTLY ONE disposable photo. Then tap Continue G5.1.");
        runButton.setText("I captured ONE test photo — Continue G5.1");
    ...
    private void finishSuccess(){
        ...
        append("G5.1 RESULT: ONE DISPOSABLE JPG DOWNLOADED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        baselineCatalog.clear();
        baselineReady=false;
        reportFinished=true;
        phase=Phase.COMPLETE;
        stage=Stage.BASELINE;
        setStatus("G5.1 complete. Review the report before any further media work.");
        runButton.setText("Start G5.1 baseline");
        enableActions();
    }
    ...
    private void fail(String m){if(reportFinished)return;cancelTimeout();reportFinished=true;phase=Phase.COMPLETE;append("G5.1 RESULT: FAILED — "+m);cleanupRuntime(false);baselineCatalog.clear();baselineReady=false;append("END REPORT");stage=Stage.BASELINE;setStatus(m);runButton.setText("Start G5.1 baseline");enableActions();}
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G5.1 Disposable Photo Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G5.1 Disposable Photo Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G5.1 report"));}
