package com.parkarsite.g1singlephotoprobe56;
...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Disposable Photo Probe v0.5.6"); t.setTextSize(24); root.addView(t);
        TextView n=new TextView(this); n.setText("G5.6: baseline the safe catalog, exit transfer, then ARM the BLE visibility watch BEFORE taking exactly one photo. P2P/catalog/media access is blocked until a +1 image inventory is confirmed."); n.setPadding(0,dp(8),0,dp(16)); root.addView(n);
        statusView=new TextView(this); statusView.setTextSize(16); root.addView(statusView);
        runButton=new Button(this); runButton.setText("Start G5.6 baseline"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then start the G5.6 baseline.");
    }
    ...
    private void startTransportCycle(boolean fresh){
        ...
        if(fresh){
            ...
            append("App version: 0.5.6");
            append("Gate: G5.6 visibility-gated single disposable JPG download");
            ...
            if(!baselineReady||baselineCatalog.isEmpty()){
                fail("Baseline is unavailable. Restart G5.6 from Phase A.");
                return;
            }
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            String reason=failureReason==null?"G5.6 did not complete.":failureReason;
            cleanupRuntime(false);
            append("");
            append("SUMMARY");
            ...
            append("G5.6 RESULT: FAILED — "+reason);
            append("END REPORT");
            baselineCatalog.clear();
            baselineReady=false;
            reportFinished=true;
            ...
            stage=Stage.BASELINE;
            setStatus(reason);
            runButton.setText("Start G5.6 baseline");
            enableActions();
            return;
        }
    ...
    private void finishSuccess(){
        ...
        append("G5.6 RESULT: ONE DISPOSABLE JPG DOWNLOADED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        baselineCatalog.clear();
        baselineReady=false;
        reportFinished=true;
        phase=Phase.COMPLETE;
        stage=Stage.BASELINE;
        setStatus("G5.6 complete. Review the report before any further media work.");
        runButton.setText("Start G5.6 baseline");
        enableActions();
    }
    ...
    private void fail(String m){if(reportFinished)return;cancelTimeout();reportFinished=true;phase=Phase.COMPLETE;append("G5.6 RESULT: FAILED — "+m);cleanupRuntime(false);baselineCatalog.clear();baselineReady=false;append("END REPORT");stage=Stage.BASELINE;setStatus(m);runButton.setText("Start G5.6 baseline");enableActions();}
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G5.6 Disposable Photo Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G5.6 Disposable Photo Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G5.6 report"));}
