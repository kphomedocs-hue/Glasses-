...
public final class MainActivity extends Activity {
    ...
    private enum Phase { IDLE, COUNT_SENT, ENTER_SENT, DISCOVERING, CONNECTING, CONNECTED, CATALOG_DELAY, CATALOG_GET, MEDIA_GET, EXIT_SENT, CLEANUP, COMPLETE }
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Disposable Photo Probe v0.5.2"); t.setTextSize(24); root.addView(t);
        ...
        runButton=new Button(this); runButton.setText("Start G5.2 baseline"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then start the G5.2 baseline.");
    }
    ...
    private void startTransportCycle(boolean fresh){
        ...
        if(fresh){
            ...
            append("App version: 0.5.1");
            append("Gate: G5.2 disposable JPG download with exact Cyan inventory + catalog-readiness parity");
            ...
            if(!baselineReady||baselineCatalog.isEmpty()){
                fail("Baseline is unavailable. Restart G5.2 from Phase A.");
                return;
            }
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            String reason=failureReason==null?"G5.2 did not complete.":failureReason;
            cleanupRuntime(false);
            append("");
            append("SUMMARY");
            ...
            append("G5.2 RESULT: FAILED — "+reason);
            append("END REPORT");
            baselineCatalog.clear();
            baselineReady=false;
            reportFinished=true;
            ...
            stage=Stage.BASELINE;
            setStatus(reason);
            runButton.setText("Start G5.2 baseline");
            enableActions();
            return;
        }
    ...
    private void finishBaselinePhase(){
        ...
        phase=Phase.COMPLETE;
        setStatus("Phase A complete. Now use the glasses to capture EXACTLY ONE disposable photo. Then tap Continue G5.2.");
        runButton.setText("I captured ONE test photo — Continue G5.2");
    ...
    private void finishSuccess(){
        ...
        append("G5.2 RESULT: ONE DISPOSABLE JPG DOWNLOADED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        baselineCatalog.clear();
        baselineReady=false;
        reportFinished=true;
        phase=Phase.COMPLETE;
        stage=Stage.BASELINE;
        setStatus("G5.2 complete. Review the report before any further media work.");
        runButton.setText("Start G5.2 baseline");
        enableActions();
    }
    ...
    private void fail(String m){if(reportFinished)return;cancelTimeout();reportFinished=true;phase=Phase.COMPLETE;append("G5.2 RESULT: FAILED — "+m);cleanupRuntime(false);baselineCatalog.clear();baselineReady=false;append("END REPORT");stage=Stage.BASELINE;setStatus(m);runButton.setText("Start G5.2 baseline");enableActions();}
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G5.2 Disposable Photo Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G5.2 Disposable Photo Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G5.2 report"));}
