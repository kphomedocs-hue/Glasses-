...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        title.setText("K G1 Capture Visibility Probe v0.5.3");
            ...
            if(cmd==0x73){
                ...
                if(eventId==0x01){
                    ...
                    if(inv!=null){
                        ...
                        if(passive01Logged<10){
                            passive01Logged++;
                            append("Passive 0x73/0x01 inventory: images="+inv.imageCount+", videos="+inv.videoCount+", recordings="+inv.recordCount+", configFileType="+inv.configFileType+", AP-only="+tri(inv.onlySupportApKnown,inv.onlySupportApImport));
                        }
                    }
                }
            }
    ...
    private void maybeCompleteQuery(){
        ...
        if(stage==Stage.BASELINE_QUERY){
            ...
            append("Now capture EXACTLY ONE disposable photo with the glasses.");
            append("After the capture, tap the button below.");
            stage=Stage.WAIT_CAPTURE;
            runButton.setText("I captured ONE photo — Start 60s watch");
            runButton.setEnabled(true);
            setStatus("Baseline complete. Capture exactly ONE photo now, then tap the button.");
    ...
    private void beginPostCaptureWatch(){
        if(stage!=Stage.WAIT_CAPTURE||baselineInventory==null||reportFinished)return;
        stage=Stage.POST_CAPTURE_OBSERVE;
        runButton.setEnabled(false);
        append("");
        append("POST-CAPTURE VISIBILITY WATCH");
        append("User confirmation: exactly one photo captured");
        append("BLE remained connected: YES");
        append("Transfer mode: OFF");
        append("Observation window after confirmation: 60 seconds");
        append("Additional proprietary writes during watch: 0");
        setStatus("Watching passive 0x73 notifications for 60 seconds…");
        observeTask=()->{
            observeTask=null;
    ...
    private void finishVisibilityReport(){
        ...
        append("Passive 0x73 notifications observed: "+passive73Count);
        append("Passive 0x73/0x01 inventory events observed: "+passive01Count);
        append("Observed 0x73 event IDs: "+eventIdSummary());
        append("Maximum passive image count observed: "+(maxPassiveImage<0?"<none>":Integer.toString(maxPassiveImage)));
        append("Passive image increase observed: "+(passiveVisibilitySeen?"YES":"NO"));
