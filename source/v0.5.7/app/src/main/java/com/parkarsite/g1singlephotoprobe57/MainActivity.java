package com.parkarsite.g1singlephotoprobe57;
...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 Disposable Photo Probe v0.5.7"); t.setTextSize(24); root.addView(t);
        TextView n=new TextView(this); n.setText("G5.7: controlled one-photo transfer diagnostic with explicit stage timing, +1 inventory gate, single catalog read, one media GET, JPEG validation and temporary-file cleanup."); n.setPadding(0,dp(8),0,dp(16)); root.addView(n);
        statusView=new TextView(this); statusView.setTextSize(16); root.addView(statusView);
        runButton=new Button(this); runButton.setText("Start G5.7 baseline"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then start the G5.7 baseline.");
    }
    ...
    private void startTransportCycle(boolean fresh){
        ...
        if(fresh){
            ...
            append("App version: 0.5.7");
            append("Gate: G5.7 visibility-gated single disposable JPG download");
            append("Selection: BASELINE CATALOG -> ARM BLE VISIBILITY -> ONE JPG -> CONFIRM +1 INVENTORY -> P2P -> EXACTLY ONE NEW SAFE JPG");
            ...
            if(!baselineReady||baselineCatalog.isEmpty()){
                fail("Baseline is unavailable. Restart G5.7 from Phase A.");
                return;
            }
    ...
    private void startMediaGet(String ip,String candidate){
        ...
        new Thread(()->downloadMediaOnce(safeIp,safeCandidate),"g5-1-one-media-get").start();
    }

    private void downloadMediaOnce(String ip,String candidate){
        HttpURLConnection c=null;
        File localFile=null;
        boolean tempCleanupPass=false;
        try{
            ...
            localFile=new File(getCacheDir(),"g5_disposable_"+System.currentTimeMillis()+".jpg");
            ...
            if(!tempCleanupPass){
                postMediaFailure("Validated temporary file could not be removed.");
                return;
            }
            mediaDoneAtMs=SystemClock.elapsedRealtime();
            final boolean cleanupResult=tempCleanupPass;
            ...
            handler.post(()->{
                ...
                append("Content-Length consistency: "+(m.declaredLength<0?"NOT PROVIDED":(m.bytes==m.declaredLength?"PASS":"FAIL")));
                ...
                append("Temporary file cleanup after validation: "+(cleanupResult?"PASS":"FAIL"));
                if(catalogDoneAtMs>=0&&mediaDoneAtMs>=0)append("Catalog parsed -> media validated: "+(mediaDoneAtMs-catalogDoneAtMs)+" ms");
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            String reason=failureReason==null?"G5.7 did not complete.":failureReason;
            cleanupRuntime(false);
            append("");
            append("SUMMARY");
            ...
            append("G5.7 RESULT: FAILED — "+reason);
            append("END REPORT");
            baselineCatalog.clear();
            baselineReady=false;
            reportFinished=true;
            ...
            stage=Stage.BASELINE;
            setStatus(reason);
            runButton.setText(physicalRunStarted?"Run ended — restart app":"Start G5.7 baseline");
            enableActions();
    ...
    private void finishSuccess(){
        ...
        append("G5.7 RESULT: ONE DISPOSABLE JPG DOWNLOADED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        baselineCatalog.clear();
        baselineReady=false;
        reportFinished=true;
        phase=Phase.COMPLETE;
        stage=Stage.BASELINE;
        setStatus("VERIFIED — one disposable JPG transfer completed. Restart app for another run.");
    ...
    private void fail(String m){if(reportFinished)return;cancelTimeout();reportFinished=true;phase=Phase.COMPLETE;append("G5.7 RESULT: FAILED — "+m);cleanupRuntime(false);baselineCatalog.clear();baselineReady=false;append("END REPORT");stage=Stage.BASELINE;setStatus(m);runButton.setText("Start G5.7 baseline");enableActions();}
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G5.7 Disposable Photo Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G5.7 Disposable Photo Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G5.7 report"));}
