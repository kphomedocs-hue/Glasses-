...
JAVA="$SRC/java/com/parkarsite/g1singlephotoprobe/MainActivity.java"
MANIFEST="$SRC/AndroidManifest.xml"
fail=0
bad(){ if grep -RniE "$1" "$SRC" >/tmp/g5_hits 2>/dev/null; then echo "FAIL: $2"; cat /tmp/g5_hits; fail=1; else echo "PASS: $2 absent"; fi; }
...
bad 'setRequestProperty\([^\n]*(Range|range)' 'Range/resume request header'
bad 'setRequestProperty\(' 'custom HTTP request headers'
...
grep -Fq 'private static final int CATALOG_MAX_BYTES = 65536;' "$JAVA" || { echo "FAIL: catalog cap missing"; fail=1; }
...
grep -Fq 'new FileOutputStream(file)' "$JAVA" || { echo "FAIL: streaming file output missing"; fail=1; }
grep -Fq 'm.first[0]==0xFF&&m.first[1]==0xD8&&m.first[2]==0xFF' "$JAVA" || { echo "FAIL: JPEG SOI validation missing"; fail=1; }
grep -Fq 'prev==0xFF&&last==0xD9' "$JAVA" || { echo "FAIL: JPEG EOI validation missing"; fail=1; }
grep -Fq 'declared>=0&&m.bytes!=declared' "$JAVA" || { echo "FAIL: Content-Length validation missing"; fail=1; }
