...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView t=new TextView(this); t.setText("K G1 P2P IP Notify Probe"); t.setTextSize(24); root.addView(t);
        TextView n=new TextView(this); n.setText("G4A2: repeats the proven G4A P2P association flow and exposes only sanitized 0x73 event IDs. Event 0x08 may resolve the glasses IPv4. No new proprietary query, HTTP, socket or media access."); n.setPadding(0,dp(8),0,dp(16)); root.addView(n);
        statusView=new TextView(this); statusView.setTextSize(16); root.addView(statusView);
        runButton=new Button(this); runButton.setText("Run G4A2 IP notify probe"); runButton.setOnClickListener(v->begin()); root.addView(runButton);
    ...
    private void renderIdle(){
        ...
        statusView.setText("Ready. Force-stop Cyan Glasses, keep AIMB-G1 paired, then run G4A2.");
    }
    ...
    private void runProbe(){
        ...
        expectedP2pName=null; credentialPassword=null; failurePending=false; ipEventObserved=false; failureReason=null; glassesClientIp=null; observed73Events.clear(); phase=Phase.IDLE;
        runButton.setEnabled(false);copyButton.setEnabled(false);shareButton.setEnabled(false);
        append("K G1 P2P-IP NOTIFY PROBE REPORT");
        append("Generated: "+isoNow());
        append("App version: 0.4.1");
        append("Gate: G4A2 passive glasses P2P-IP notification capture only");
        append("BLE writes allowed: P2P enter once + transfer exit once");
        append("0x73 reporting: EVENT ID ONLY; event 0x08 may expose IPv4");
    ...
    private void finishAfterCleanup(){
        if(failurePending){
            ...
            cleanupRuntime(false);
            append("");append("SUMMARY");append("Exact target peer matched: "+targetPeerMatched);appendIpSummary();append("HTTP/socket/media operations: 0");append("Credentials logged/persisted: NO");append("G4A2 RESULT: FAILED — "+reason);append("END REPORT");
    ...
    private void finishSuccess(){
        cleanupRuntime(false); append("");append("SUMMARY");append("Exact target peer matched: "+targetPeerMatched);append("P2P group formed: TRUE");appendIpSummary();append("HTTP/socket/media operations: 0");append("Credentials logged/persisted: NO");
        append(glassesClientIp==null?"G4A2 RESULT: NO IP RESOLUTION — STOP AND REVIEW":"G4A2 RESULT: GLASSES IP RESOLVED — REVIEW PHYSICAL REPORT");
        append("END REPORT");
        reportFinished=true;phase=Phase.COMPLETE;setStatus("G4A2 notify probe complete. Review the report.");enableActions();
    }
    ...
    private void copy(){ClipboardManager c=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(c!=null){c.setPrimaryClip(ClipData.newPlainText("G4A2 P2P IP Notify Report",report.toString()));Toast.makeText(this,"Report copied",Toast.LENGTH_SHORT).show();}}
    private void share(){Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_SUBJECT,"AIMB-G1 G4A2 P2P IP Notify Report");i.putExtra(Intent.EXTRA_TEXT,report.toString());startActivity(Intent.createChooser(i,"Share G4A2 report"));}
